...
General-purpose enterprise / consulting theme. Light canvas, restrained blue
accents, compact cards, strong tables, and executive-readable information
density. Designed for strategy updates, market research, operating reviews,
business cases, and management presentations that need to look serious without
feeling heavy.

## When to use this theme
- Strategy updates, management reviews, consulting deliverables.
- Market research, business cases, product/operations reviews.
- Decks that must work on projector, PDF, and printed handout.

## When NOT to use
- Cinematic product launches or lifestyle marketing decks.
- Deep technical incident reviews where terminal/code aesthetics matter more.
...
- `bg-canvas` — light neutral workspace background.
- `bg-card` — white card / table surface fill.
- `brand-primary` — enterprise blue, primary accents and KPI values.
- `brand-deep` — deep blue, section markers and strong emphasis.
- `text-strong` — near-black body and title text.
- `text-muted` — labels, captions, page numbers.
- `accent` — teal, used sparingly for secondary emphasis.
- `divider` — hairline color for separators and outlines.
- `font-latin` — Latin: Aptos → Inter → Source Sans 3 → IBM Plex Sans → Helvetica → Arial. Aptos matches modern Office; Inter/Source Sans keep exported decks crisp.
