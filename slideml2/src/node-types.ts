...
const STACK_DETAILED: Record<string, NodeFieldInfo> = {
  ...
  optional: { valueType: "boolean", description: "Marks this child as the first candidate for the agent to shorten/remove when capacity fails. It is NOT auto-dropped by default." },
  autoDrop: { valueType: "boolean", description: "Explicitly allow the renderer to drop this optional child during fallback. Use only for decorative/non-semantic extras." },
  dropWhenTight: { valueType: "boolean", description: "Alias for autoDrop:true; explicitly allows dropping this optional child when its parent is over capacity." },
...
const TABLE_DETAILED: Record<string, NodeFieldInfo> = {
  ...
  cellPadding: { valueType: "object", description: "Default cell padding. Decimal values <=1.6 are cm; common integer values like 6/8 are treated as points. Object form supports {left/right/top/bottom}." },
...
const CHART_DETAILED: Record<string, NodeFieldInfo> = {
  ...
  encoding: { valueType: "object", description: "Binding encoding {x, y, orientation?, series?, seriesName?, seriesOptions?}; y may be a string or string[]. label/value are aliases for x/y, especially for pie and doughnut charts. seriesOptions can set combo type, secondary axis, trendLine, and errorBars per output series." },
  ...
  plotArea: { valueType: "object", description: "Manual plot-area layout {x,y,w,h} for tight chart compositions. Prefer 0..1 factors; cm-like values are accepted and converted to chart-frame factors." },
  ...
  dataLabels: { valueType: "object", description: "{show?, position?: 'bestFit'|'center'|'insideEnd'|'insideBase'|'outsideEnd', showValue?, showCategoryName?, showSeriesName?, showPercent?, showLegendKey?, showLeaderLines?, minPercent?}; pie defaults to native PowerPoint outsideEnd category+percent labels with leader lines; doughnut uses repair-safe external PPT text labels/leader lines instead of native dLblPos; slices below 3% are suppressed" },
...
const SHAPE_DETAILED: Record<string, NodeFieldInfo> = {
  ...
  text: { valueType: "object", description: "Optional text inside the shape. Accepts a string or {text, align?, color?, fontSize?, fontWeight?, fontFamily?}." },
