 ...
 * `position:"bottom-right"` (or anchor) expecting a small footer-corner
 ...
 * image+position) out of the content stack. They stay at slide level so
...
function findImage(shapes: Array<{ type: string; xfrm?: { x: number; y: number; cx: number; cy: number }; src?: string; name?: string }>, src: string) {
...
describe("yajush: footer/corner image placement (slide-level overlays)", () => {
  it("image with position:bottom-right at slide level renders at bottom-right corner, not stretched", () => {
    const slide: SlideV2 = {
      id: "history",
      title: "历史",
      children: [
  ...
  it("flow children (text/etc.) still wrap into the content stack alongside an overlay image", () => {
    const slide: SlideV2 = {
      ...
      children: [
        ...
        { id: "mixed.seal", type: "image", src: "/tmp/seal.png", position: "bottom-right", width: 2.0, height: 1.0 },
      ],
    };
    const ast = renderToAst(sourceToRenderedDeck(deck([slide])));
      ...
      children: [
        {
          ...
          children: [
            ...
            { id: "explicit.inline-img", type: "image", src: "/tmp/inline.png", position: "bottom-right" },
          ],
        },
      ],
    };
