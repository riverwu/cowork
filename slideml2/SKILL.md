...
- `org-chart` — People/role reporting hierarchy with variable-size tree layout, adaptive card sizes, personnel-list detail, auto-spreading gaps inside the available region, and elbow connectors. type='org-chart' required={nodes:[{id?,name|label|title, role|position?, team?, body|description?, people|members|personnel?, parent|reportsTo?, level?, tone?, size?|width?|height?}]} optional={links, title, density, detail:auto|compact|full, maxChildrenPerParent, treeMaxWidth, treeMaxHeight, spread=true, tone, variant, surface} capacity="2-5 readable levels; pass treeMaxWidth/treeMaxHeight or fixedWidth/fixedHeight when placing the tree in a smaller region; pass detail only where useful because dense lower levels become title-only; split by function when ORG_OVERFLOW appears"
- `hierarchy-tree` — Generic non-people hierarchy/tree. type='hierarchy-tree' required={nodes:[{id?,label|title|name, body?, parent?, level?, tone?}]} optional={links, title, density, tone, variant, surface}
...
- `pyramid` — Tiered hierarchy / strategy stack. type='pyramid' required={levels:[{label|title|name, body?, value?, tone?}]} optional={title, orientation:top-down|bottom-up, density, tone, variant, surface}
...
| Process / value chain / workflow        | `process-flow`                 | `value-chain`, `cycle-diagram`, `stat-flow`, `funnel`, `arrow-link` |
...
| Org / hierarchy / decision routing      | `org-chart`                    | `hierarchy-tree`, `decision-tree`, `raci-matrix` |
| Stakeholder / governance / work status  | `stakeholder-map`              | `raci-matrix`, `kanban-board`, `checklist`      |
| Architecture / layered system map       | `architecture-map`             | `value-chain`, `hierarchy-tree`, `legend`       |
