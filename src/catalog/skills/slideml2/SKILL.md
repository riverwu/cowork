...
1. **Task contract first.** Before `create_deck`, write a compact contract in your reasoning: audience, decision/job, source material, slide count, tone/theme, required assets, chart plan, and validation bar. For business / research report decks, read [business.md](business.md) completely before planning; do not infer business style from a partial read.
2. **Outline next.** Produce a slide outline with one job per slide, short titles, and 1-3 regions. Avoid chrome-heavy structures; split dense pages early.
3. **Component plan before JSON.** For each slide, decide the page job and primary component before writing `children`. If unsure, call `describe_schema({components:[...]})` for the 2-4 candidate components. Do not start from `text` boxes and coordinates.
4. **Asset pass before slides.** Decide needed backgrounds, illustrations, icons, screenshots, data charts, and external visuals. Generate or prepare assets first, save them under the current run assets directory, and reference absolute paths. For icon sets, call `generate_icon_sheet` before writing slides; see **Generated Icon Assets** below. Use SlideML2 structured charts/tables when precision matters; when a MatPilot/Canvas/chart-rendering tool is available and the chart is visually complex, generate a chart image there, then place it via `image-card`/`chart-with-rail` with `fit:"contain"`.
...
6. **Write slides in checkpoint batches.** Use `replace_slide`, `insert_slide`, `delete_slide`, or focused `patch_deck({set?,unset?,insert?,move?,copy?})`. `replace/insert.slide` is only `{id,title?,background?,children,notes?,metadata?}`. `patch_deck` groups are path maps except `unset` (array); ops apply `unset → set → insert → move → copy`.
7. **Validate before continuing.** After every 1-2 slide writes, immediately run `validate_render({deckPath,render:true})`; repair blocking diagnostics with `read_deck` on affected slides before writing more slides. Do not add the whole deck first and then validate: early diagnostics teach sizing, density, and component choices for the next batch. Finish only after the PPTX renders with no blocking diagnostics.
...
Use this loop for every content slide before calling `replace_slide` or `insert_slide`:
...
5. After validation, if a slide contains many `text` nodes with `at`, redesign it with a semantic component unless it is a cover, poster layout, image annotation, or precise overlay.
...
- Never mutate the SlideML2 source deck with `write_file`, `run_node`, or `run_python`. If a slide write fails, retry `replace_slide`/`insert_slide` with `slide` as an object literal, or use `patch_deck`; if `validate_render` fails, repair the SlideML2 source instead of generating a separate PPTX with python-pptx.
...
- Pass `style` and `palette` from the deck theme, e.g. `premium standalone business line icons, rounded geometry, consistent stroke, no tile/card background` and `["#111827","#2563EB","#94A3B8"]`. Keep 4-12 icons per sheet; set `grid` to match (`3x2`, `4x3`, etc.).
- After the tool returns, use the returned manifest paths, not the sheet image. Place icons as `feature-card.iconSrc:"/abs/.../assets/icons/bank.png"` or as `image`/`image-card` with `fit:"contain"`.
...
- process-flow: Connected workflow/pipeline/causal sequence; use when movement matters. Steps support status?:enum[brand|positive|warning|danger|neutral], owner?:string, time, icon, bullets. kind=semantic parent=stack children=none type='process-flow' required={steps:array of {title/label:string, body/description:string, status?:enum[brand|positive|warning|danger|neutral], owner?:string, time?:string, icon?:string, bullets?:array} or items} optional={direction:enum[horizontal|vertical], variant:enum[plain|cards], density:enum[comfortable|compact], surface:object} example={"type":"process-flow","variant":"cards","steps":[{"title":"Collect","status":"positive","owner":"Research"}]}
- timeline: Chronological sequence with dates, eras, milestones, or releases; use only when time organizes meaning. Item content forms: `body` for one-sentence captions, `content` for any DomNode such as metric-card/insight-card/image/quote/chart-card/stack, or title-only for compact lanes. Use `direction:"horizontal"` for ≤5 simple text items; use `vertical` when any item has rich `content` or more detail. Split beyond 6 simple items, 3 metric-card items, or 2-3 insight-card items. kind=semantic parent=stack children=none type='timeline' required={items:array of {time/date/year:string}, where each item also has at least one of title/label/headline/name/body/content} optional={direction:enum[horizontal|vertical], orientation:enum[horizontal|vertical] (alias), per-item: title/label/headline/name, body/description (text), content (any DomNode — metric-card, insight-card, image, quote, chart-card, stack, etc.)} example={"type":"timeline","direction":"vertical","items":[{"time":"2026 Q1","content":{"type":"metric-card","value":"$15M","label":"Series A"}}]}
