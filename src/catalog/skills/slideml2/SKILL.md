...
The slide canvas is **25.4 × 14.29 cm** (16:9). All distances below are cm.
...
- insight-card: One modular insight with badge/headline/detail/proof bullets. Use for a curated finding or recommendation, not generic paragraph storage. Do not fill many consecutive slides with insight-card grids; switch to key-takeaway, evidence-layout, chart-card, table-card, comparison-card, process-flow, numbered-grid, or takeaway-list when the slide job is verdict, proof, comparison, process, or summary. kind=semantic parent=grid children=none type='insight-card' required={headline:string or title:string} optional={badge:string, detail:string, body:string, description:string, bullets/items/points:array, tone:enum[neutral|brand|positive|warning|danger]} example={"type":"insight-card","headline":"Finding","bullets":["Proof"]}
