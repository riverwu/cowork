...
Composable structural blocks:

- `section-header` — Local heading block for a slide region. Use instead of loose eyebrow/title/rule text stacks. type='section-header' required={title} optional={eyebrow, subtitle/body, level:h1|h2|compact, align:left|center|right, rule, variant:plain|minimal, tone}
- `figure` — Evidence wrapper for chart/image/table/diagram plus caption/source. Flat shortcuts expand to flexible raw image/chart/table primitives. type='figure' optional={title, content/evidence/visual/media:DomNode, src:image-ref, chartType+labels+series, rows+headers, caption, source, fit:cover|contain|fill, variant:frameless|card|compact, tone, surface}
- `rail-block` — Compact rail section for interpretation, constraints, actions, or short bullets. Use inside `side-rail` or narrow split regions. type='rail-block' optional={title/headline, body/detail, items, footer, tone, variant:plain|accent|panel, density, surface, children}
- `evidence-block` — Local claim + proof + interpretation block. Use when one region needs a compact argument. type='evidence-block' required={claim} optional={proof/evidence/visual/content:DomNode, proofText, interpretation/detail, source, tone, variant:plain|panel|compact, surface}
- `key-value-list` — Aligned key/value facts for metadata, assumptions, parameters, or compact facts. type='key-value-list' required={items:[{key|label|term|name,value,detail?,tone?}]} optional={pairs/rows/facts aliases, title, columns:1|2, density, variant:plain|table|panel, tone, surface}
- `module-strip` — Lightweight strip/grid of 2–5 peer modules beneath or beside a main object. type='module-strip' required={items:[DomNode | {title|label|name, body|detail, value?, badge?, tone?, marker?}]} optional={title, columns, gap, variant:plain|subtle|cards, density, tone, surface}
...
- `chart-with-rail` — Dominant chart/table/evidence plus a narrow rail. Use either nested `evidence:{type:"chart-card"|...}` or flat chart aliases `chartType + chartData:{labels,series}`. A text-like `evidence` beside flat chart data becomes a rail proof/source note, not the main evidence. Pie/doughnut charts get distinct default slice colors when `colors` is omitted. Use `railBody:"..."` for one concise interpretation; use `railBody:[{type:"text",...},{type:"table-card",density:"compact",...}]` or `rail:{type:"side-rail",children:[...]}` when the rail needs compact structured support. type='chart-with-rail' required={evidence OR chartType+chartData} optional={rail, headline/detail, railTitle/railBody:string|DomNode[], keyTakeaway:{headline,detail,tone}, items, chartTitle, chartTone, colors, showLegend, showValues, yFormat, layout:rail-right|rail-left|stacked, ratio default [0.72,0.28] or stacked [0.68,0.32], gap, tone} capacity="chart body >=4.8x3.0cm; rail <=30% width; stack when rail text is long"
...
- `comparison-table` — Multi-option matrix; features rows, options columns. Accepts canonical `features + options:[{name,values,recommended?}]`, natural row records `options:["A","B"], rows:[{feature:"ARR", A:"$1m", B:"$2m"}]`, and agent-natural object features `options:["A","B"], features:[{feature:"ARR", A:"$1m", B:"$2m"}]`; if rows carry feature labels and option-name keys, `features/options` can be inferred. type='comparison-table' required={features+options OR rows/features with feature+option columns} optional={rows, title} capacity="3-8 features x 2-6 options; 5-6 options render compact; split when cells need sentences"
