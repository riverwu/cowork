...
import {
  IconArrowLeft, IconClock, IconChart, IconTaskList,
  IconMail, IconTrend, IconFolder, IconPackage, IconPlus, IconWarning,
      ...
      <div className="flex h-full">
        <div className="flex-1 flex flex-col min-w-0">
          <div className="flex items-center px-6 py-2 border-b border-[var(--border)]">
            ...
            >
              <IconArrowLeft size={14} /> {t("home.backToHome")}
            </button>
          </div>
          ...
          <div className="space-y-5">
            <section>
              ...
              <div className="bg-[var(--surface-lowest)] rounded-xl border border-[var(--border)] overflow-hidden shadow-[var(--shadow-sm)]">
                <AttentionItem type="warning" title="客诉升级预警" tag="待审阅" desc="系统检测到本周客诉量上升了 23%，触发阈值预警。建议及时…" action="立即查看" />
                <AttentionItem type="error" title="项目周报" desc="上周报告提取 Jira 数据时，连接异常导致数据不完整。" action="查看详情" last />
              </div>
            </section>
            ...
            <section>
              ...
              <div className="bg-[var(--surface-lowest)] rounded-xl border border-[var(--border)] overflow-hidden shadow-[var(--shadow-sm)]">
                <OutputItem time="今天 09:00" title="周度销售分析报告" status="报告已生成" statusColor="success" />
                <OutputItem time="昨天 14:30" title="CRM数据同步报告" status="已发送至群聊" statusColor="success" />
                <OutputItem time="昨天 09:00" title="用户调研数据清洗" status="处理完成" statusColor="success" last />
                <div className="px-4 py-2.5 text-center border-t border-[var(--border)]">
                  <button className="text-[12px] text-[var(--primary-accent)] hover:underline cursor-pointer">查看全部产出 →</button>
                </div>
              </div>
            </section>
          </div>
          ...
          <section>
            <div className="flex items-center justify-between mb-3">
              <h2 className="flex items-center gap-2 text-[13px] font-semibold text-[var(--on-surface)]">
                <IconPackage size={15} /> {t("home.myApps")}
              </h2>
              <button className="text-[12px] text-[var(--primary-accent)] hover:underline cursor-pointer">管理</button>
            </div>

            <div className="grid grid-cols-2 gap-3">
              <AppCard icon={<IconChart size={18} />} iconBg="bg-blue-50 text-blue-600" title="每周销售分析" desc="自动汇总各品类销售数据，识别异常变化并生成管理层简报。" schedule="每周一 08:00" />
              <AppCard icon={<IconTaskList size={18} />} iconBg="bg-indigo-50 text-indigo-600" title="项目进度追踪" desc="提取 Jira 数据，整理 Sprint 进展并以任务优先级排列。" schedule="每日自动运行" />
              <AppCard icon={<IconMail size={18} />} iconBg="bg-teal-50 text-teal-600" title="客户满意度监测" desc="实时抓取支持工单和客户反馈，进行情感分析并汇总关键指标。" schedule="每日自动运行" />
              <AppCard icon={<IconTrend size={18} />} iconBg="bg-orange-50 text-orange-600" title="商机线索提醒" desc="监控 CRM 中的新增线索，通过 AI 进行商机价值评估和优先…" schedule="实时监控" />

              {sources.map((s) => (
                <AppCard key={s.id} icon={<IconFolder size={18} />} iconBg="bg-gray-50 text-gray-600" title={s.name} desc={t("knowledge.knowledgeSource")} schedule={s.status} />
              ))}
              ...
              <div className="border border-dashed border-[var(--border)] rounded-xl p-5 flex flex-col items-center justify-center text-center min-h-[140px] hover:bg-[var(--surface-low)] hover:border-[var(--on-surface-tertiary)] transition-all cursor-pointer group">
                <div className="w-9 h-9 rounded-lg bg-[var(--surface-container)] group-hover:bg-[var(--primary-accent-light)] flex items-center justify-center mb-2 transition-colors text-[var(--on-surface-tertiary)] group-hover:text-[var(--primary-accent)]">
                  <IconPlus size={18} />
                </div>
                <span className="text-[12px] text-[var(--on-surface-tertiary)] group-hover:text-[var(--on-surface-secondary)]">{t("home.createApp")}</span>
              </div>
            </div>
          </section>
        </div>
...
function AttentionItem({ type, title, tag, desc, action, last }: { type: "warning" | "error"; title: string; tag?: string; desc: string; action: string; last?: boolean }) {
  return (
    <div className={`px-4 py-3 ${last ? "" : "border-b border-[var(--border)]"}`}>
      <div className="flex items-start gap-2.5">
        <span className="mt-1 text-[var(--on-surface-tertiary)]">
          <IconWarning size={14} />
        </span>
...
function OutputItem({ time, title, status, statusColor, last }: { time: string; title: string; status: string; statusColor: "success" | "warning" | "error"; last?: boolean }) {
  const colors = { success: "text-[var(--success)]", warning: "text-[var(--warning)]", error: "text-[var(--error)]" };
  return (
...
function AppCard({ icon, iconBg, title, desc, schedule }: { icon: React.ReactNode; iconBg: string; title: string; desc: string; schedule: string }) {
  return (
    <div className="bg-[var(--surface-lowest)] border border-[var(--border)] rounded-xl p-4 hover:shadow-[var(--shadow-md)] hover:border-[var(--surface-high)] transition-all cursor-pointer group">
      ...
      <div className="flex items-center justify-between">
        <span className="flex items-center gap-1 text-[11px] text-[var(--on-surface-tertiary)]"><IconClock size={12} /> {schedule}</span>
        <button className="text-[11px] text-[var(--primary-accent)] opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer">运行</button>
      </div>
    </div>
  );
}
