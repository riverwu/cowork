...
describe("parseJsonLenient", () => {
  it("parses well-formed JSON unchanged", () => {
    ...
    expect(parseJsonLenient('[{"pattern":"single-focus","regions":{"main":{"component":"cover","props":{"title":"x"}}}}]')).toEqual([
      { pattern: "single-focus", regions: { main: { component: "cover", props: { title: "x" } } } },
    ]);
  });
  ...
  it("repairs raw newlines inside string values (the common failure)", () => {
    ...
    const broken = `[{"pattern":"dashboard","regions":{"main":{"component":"dashboard","props":{"tl":{"body":"line one
line two
适用：A"}}}}}]`;
    const result = parseJsonLenient<Array<{ regions: { main: { props: { tl: { body: string } } } } }>>(broken);
    expect(result[0]!.regions.main.props.tl.body).toBe("line one\nline two\n适用：A");
  });
