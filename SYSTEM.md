...
- Do not use `shell` to run `node script.js` for generated deliverables. Installed skill CLIs that are explicitly documented as `node .../runtime/bin/...` commands are different: run those directly with `shell` so each CLI result remains visible.
